...
namespace FairyGUI.Utils
{
	...
	public class HtmlParser
	{
		...
		virtual public void Parse (string aSource, TextFormat defaultFormat, List<HtmlElement> elements, HtmlParseOptions parseOptions)
		{
			...
			bool ignoreFontSize = parseOptions.ignoreFontSize;
			var maxFontSize = parseOptions.maxFontSize;
			var minFontSize = parseOptions.minFontSize;
			...
			while (XMLIterator.NextTag())
			{
				...
				var tagName = XMLIterator.tagName;
				if (parseOptions.IsIgnoreTag(tagName))
				{
					...
					case "font":
						if (XMLIterator.tagType == XMLTagType.Start)
						{
							...
							if (!ignoreFontSize)
							{
								...
								if (maxFontSize > 0)
								{
									_format.size = Mathf.Min(_format.size, maxFontSize);
								}
								if (minFontSize > 0)
								{
									_format.size = Mathf.Max(_format.size, minFontSize);
								}
							}
							...
							string face = XMLIterator.GetAttribute("face");
							if (string.IsNullOrEmpty(face) == false)
							{
								_format.font = face;
								_format.fontChange = true;
								_format.offsetx = XMLIterator.GetAttribute("offsetx");
								_format.offsety = XMLIterator.GetAttribute("offsety");
							}
						}
		...
		protected void AppendText (string text)
		{
			...
			element.format.CopyFrom(_format);
			_elements.Add(element);
			if (!string.IsNullOrEmpty(element.format.offsetx)) element.Set("offsetx", element.format.offsetx);
			if (!string.IsNullOrEmpty(element.format.offsety)) element.Set("offsety", element.format.offsety);
		}
	}
}