...
namespace FairyGUI
{
   /// <summary>
    ...
    /// </summary>
    [ExecuteInEditMode]
    [AddComponentMenu("FairyGUI/UI Simulator")]
    public class UISimulator : MonoBehaviour, EMRenderTarget
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        void OnEnable()
        {
            if (Application.isPlaying)
            {
                if (this.container == null)
                {
					...
					{
						var package = UIPackage.AddPackage("Assets/Data/ui/" + packageName.Replace(".ui", ""));
						...
						if (dependencies.Length > 0)
						{
							for (int i = 0; i < dependencies.Length; i++)
							{
								...
								var _packageName = dep["name"].Replace(".ui", "");
								var package_dep = UIPackage.GetByName(_packageName);
								if (package_dep == null)
								{
									if (_packageName == "Icon")
									{
										UIPackage.AddPackage("Assets/Data/ui/Icon/" + _packageName);
									}
									else
									{
										UIPackage.AddPackage("Assets/Data/ui/" + _packageName);
									}
		...
		public void SetPartical ()
		{
			...
			foreach (var particle in particles)
			{
				...
				{
					Debug.LogError("必须挂一个特效的prefab");
					continue;
				}
				var go = Instantiate(particle.prefab);
				var wrapper = new GoWrapper(go);
				var holder = new GGraph();

				if (string.IsNullOrEmpty(particle.parentComponentName))
				{
					Debug.LogError("必须填写parent component name");
					continue;
				}
				var parent = view.GetChild(particle.parentComponentName).asCom;

				if (parent == null)
				{
					Debug.LogError("没找到 parent component name");
					continue;
				}

				switch (particle.positionOption)
				{
					...
					case PositionOption.Default:
					default:
						go.transform.position = particle.postion;
						break;
				}
		...
		[SerializeField]
		private List<ParticleList> particles;


		[System.Serializable]
		private struct ParticleList
		{
