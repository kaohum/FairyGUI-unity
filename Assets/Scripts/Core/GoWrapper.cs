...
namespace FairyGUI
{
	...
	public class GoWrapper : DisplayObject
	{
		...
		public static Transform CacheNode { get; set; } // 缓存节点
		...
		protected bool _supportAlpha = false;
		protected bool _applyGray = false;
		...
		protected static List<Renderer> helperRendererList = new List<Renderer>();
		...
		public GoWrapper ()
		{
			...
			CreateGameObject<GoWrapper>("GoWrapper");
		}
		...
		{
#if UNITY_EDITOR
			cloneMaterial = true; //Add by chenbin
#endif
			...
			if (_wrapTarget != null)
			{
				_wrapTarget.transform.SetParent(CacheNode, false);
				// 关闭旧对象的裁剪
				CloseClippingCustom();
			}
		...
		public void CacheRenderers ()
		{
			...
			_wrapTarget.GetComponentsInChildren<Renderer>(true, helperRendererList);

			int cnt = helperRendererList.Count;
			_renderers.Capacity = cnt;
			for (int i = 0; i < cnt; i++)
			{
				Renderer r = helperRendererList[i];
				Material[] mats = r.sharedMaterials;
				RendererInfo ri = new RendererInfo() { renderer = r, materials = mats, sortingOrder = r.sortingOrder };
				if (mats != null)
				{
					for (int j = 0; j < mats.Length; j++)
					{
						...
						if (mat)
						{
							if (mat.HasProperty(ShaderConfig.ID_UI_Alpha))
							{
								mat.SetFloat(ShaderConfig.ID_UI_Alpha, 1f);
								_supportAlpha = true;
							}
							if (
								mat.HasProperty(ShaderConfig.ID_Stencil) ||
								mat.HasProperty(ShaderConfig.ID_Stencil2) ||
							...
							)
							{
								...
								for (int k = 0; k < keys.Length; k++) {
								    var key = keys[i];
								    if (key == "UIPARTICLECUT") {
								    }
								}*/
								//mat.DisableKeyword("UIPARTICLECUT");
								//mat.DisableKeyword("UISTENCILCUT");
								_supportMask = true;
								//break;
							}
							...
							if (_applyGray)
							{
								mat.SetFloat(ShaderConfig.ID_GrayPhase, 1f);
							}
							else
							{
								mat.SetFloat(ShaderConfig.ID_GrayPhase, 0f);
							}
						}
					}
				}
			...
			helperRendererList.Clear();
			_renderers.Sort((RendererInfo c1, RendererInfo c2) => { return c1.sortingOrder.CompareTo(c2.sortingOrder) ; });
			...
			for (int i = 0; i < cnt; i++)
			{
				...
				for (int j = 0; j < mcnt; j++)
				{
					...
					if (!_materialsBackup.TryGetValue(mat, out newMat))
					{
						...
						newMat = Material.Instantiate(mat);
						_materialsBackup[mat] = newMat;

						if (shouldSetRQ) //Set the object rendering in Transparent Queue as UI objects
		...
		{
			if (_materialsBackup == null || _materialsBackup.Count == 0)
				return;
		...
		public void SetRenderingOrderBatching(UpdateContext context, Dictionary<int, int> wrapperSortingOrders)
		{
			...
			for (int i = 0; i < cnt; i++)
			{
				...
				if (ri.renderer != null)
				{
					int key = ri.materials[0].GetHashCode();
					if (!wrapperSortingOrders.TryGetValue(key, out int sOrder))
					{
						sOrder = context.renderingOrder++;
						wrapperSortingOrders.Add(key, sOrder);
					}
					ri.renderer.sortingOrder = sOrder;
					if (rOrder < 0) rOrder = sOrder;
				}
			}
		...
		override public void Update (UpdateContext context)
		{
			...
			// ApplyClipping(context); 已修改 2021年12月31日17:59:18 chenhr
			ApplyClippingCustom(context);

			ApplyAlphaValue(context);
			ApplyGrayValue(context);

			base.Update(context);
		}

		protected virtual void ApplyAlphaValue (UpdateContext context)
		{
			if (_supportAlpha)
			{
				int cnt = _renderers.Count;
				for (int i = 0; i < cnt; i++)
				{
					...
					if (mats != null)
					{
						...
						for (int j = 0; j < cnt2; j++)
						{
							...
							if (mat)
							{
								mat.SetFloat(ShaderConfig.ID_UI_Alpha, context.alpha * alpha);
							}
						}
					}
				}
		...
		protected virtual void ApplyGrayValue (UpdateContext context)
		{
			if (_applyGray != (context.grayed | grayed))
			{
				_applyGray = (context.grayed | grayed);
				var value = _applyGray ? 1f : 0f;
				int cnt = _renderers.Count;
				for (int i = 0; i < cnt; i++)
				{
					RendererInfo ri = _renderers[i];
					...
					if (mats != null)
					{
						...
						for (int j = 0; j < cnt2; j++)
						{
							...
							if (mat)
							{
								mat.SetFloat(ShaderConfig.ID_GrayPhase, value);
							}
						}
					}
				}
		...
		protected virtual void ApplyClippingCustom (UpdateContext context)
		{
			if (_supportMask)
			{
				int cnt = _renderers.Count;
				for (int i = 0; i < cnt; i++)
				{
					...
					if (mats != null)
					{
						...
						if (cnt2 > 0 && _cloneMaterial && ri.renderer != null
						    && !ReferenceEquals(ri.renderer.sharedMaterial, mats[0]))
							ri.renderer.sharedMaterials = mats;
					}
				}
			}
		...
		protected virtual void CloseClippingCustom ()
		{
			/*
			if (_supportMask) {
			    int cnt = _renderers.Count;
		...
		/* 已修改 2021年12月31日17:59:18 chenhr
		...
		*/
		...
		public override void Dispose ()
		{
			...
			if (_wrapTarget != null)
			{
				...
				//Debug.LogError($"要释放的 GoWrap 不为空，检查对象是否正确操作释放 => {_wrapTarget.name}, Path = {Stage.inst.GetFGUIObjPath(gOwner)}", _wrapTarget);
				_wrapTarget = null;
			}
